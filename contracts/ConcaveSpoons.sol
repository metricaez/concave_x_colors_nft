...
/**
 * @title ConcaveSpoons, TheColors contract fork
 * @dev Extends ERC721 Non-Fungible Token Standard basic implementation
 */
contract ConcaveSpoons is ERC721Enumerable, Ownable {
    ...
    constructor(IERC20 maticToken_) ERC721("The Colors (thecolors.art)", "COLORS") {
        maticToken = maticToken_;
        mintCost = 1 ether;
    }

    // Set Mint Cost. Amount will be charged in "Mint Color"
    function setMintCost (uint mintCost_) public onlyOwner {
        mintCost = mintCost_;
    }
    ...
    */
    function setProvenanceHash(string memory provenanceHash) public onlyOwner {
     ...
     /**
    * Mints color to buy who pays matic amount
    */   

    function mintColor () public { 
        require (maticToken.balanceOf(msg.sender)>mintCost, "Matic balance not enought to mint");
        maticToken.transferFrom(msg.sender,address(this), mintCost);
        mintNextColors(msg.sender);
    }

    /**
    * Mints The Colors to The Colors Legacy holders.
    * Concave fork, deleted for for numberOfTokens, just minting next color.
    * TODO: Check if compatible and testing. 
    */
    function mintNextColors(address receiver) public onlyOwner {
        require(totalSupply()  <= MAX_COLORS, "Purchase would exceed max supply of Colors");
        ...
        if (totalSupply() < MAX_COLORS) {
            tokenOwner = INFTOwner(receiver).ownerOf(mintIndex);
